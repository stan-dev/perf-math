...
static void add_pf_bench(benchmark::State& state) {
  for (auto _ : state) {
    ...
    std::vector<T> result = add_inner_pf(std::move(gen_rand_nums<T>(state)), std::move(gen_rand_nums<T>(state)));
...
int start_val = 2;
int end_val = 8192;
BENCHMARK_TEMPLATE(add_pf_bench, double)->RangeMultiplier(2)->Range(start_val, end_val)->UseManualTime();
BENCHMARK_TEMPLATE(add_pf_bench, stan::math::var)->RangeMultiplier(2)->Range(start_val, end_val)->UseManualTime();
