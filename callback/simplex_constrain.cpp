...
static void simplex_constrain(benchmark::State& state) {
  using stan::math::var;
  ...
  auto run = [](const auto&... args) {
    return sum(simplex_constrain(args...));
  };
...
int end_val = 1024;
BENCHMARK(toss_me);
BENCHMARK(simplex_constrain)->RangeMultiplier(2)->Range(start_val, end_val)->UseManualTime();
BENCHMARK_MAIN();
