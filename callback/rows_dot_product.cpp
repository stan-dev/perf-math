...
static void rows_dot_product(benchmark::State& state) {
  using stan::math::var;
  ...
  auto run = [](const auto&... args) {
    return sum(rows_dot_product(args...));
  };
...
int end_val = 1024;
BENCHMARK(toss_me);
BENCHMARK(rows_dot_product)->RangeMultiplier(2)->Range(start_val, end_val)->UseManualTime();
BENCHMARK_MAIN();
